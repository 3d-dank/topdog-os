...
interface Customer {
  id: string; name: string; email: string; appId: string; plan: string; joined: string; ltv: number; status: 'active' | 'churned' | 'trial'
}
...
function downloadCSV(data: Customer[]) {
  ...
  const rows = data.map((c) => [c.name, c.email, PORTFOLIO.find((a) => a.id === c.appId)?.name ?? c.appId, c.plan, c.joined, '$' + c.ltv.toFixed(2), c.status])
...
export default function CustomersPage() {
  ...
  const tabs = [{ id: 'all', label: 'All Apps' }, ...PORTFOLIO.map((a) => ({ id: a.id, label: a.emoji + ' ' + a.name }))]
  const filtered = activeTab === 'all' ? MOCK : MOCK.filter((c) => c.appId === activeTab)
  const topCustomers = [...MOCK].sort((a, b) => b.ltv - a.ltv).slice(0, 3)
    ...
    <div className="max-w-6xl mx-auto space-y-5">
      <div className="flex items-center justify-between" style={{ borderBottom: '1px solid #1E1E2E', paddingBottom: '12px' }}>
        ...
        <div className="flex items-center gap-3">
          <span className="mono px-2 py-1 rounded" style={{ color: '#F59E0B', backgroundColor: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.15)', fontSize: '0.6rem' }}>Sample data - Connect Stripe for live</span>
          <button onClick={() => downloadCSV(filtered)} className="mono text-xs px-3 py-1.5 rounded hover:opacity-80" style={{ color: '#64748B', backgroundColor: '#0F0F1A', border: '1px solid #1E1E2E' }}>Export CSV</button>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {topCustomers.map((c, rank) => {
            ...
            return (
              <div key={c.id} className="card-hover" style={{ backgroundColor: '#0F0F1A', border: '1px solid #1E1E2E', borderRadius: '6px', padding: '14px 16px' }}>
                <div className="flex items-start justify-between mb-2">
                  <span className="mono text-xs" style={{ color: '#334155' }}>#{rank + 1}</span>
                  <span className="mono text-lg font-bold" style={{ color: '#00D4AA' }}>${c.ltv.toFixed(2)}</span>
                </div>
                <div className="text-sm font-medium" style={{ color: '#E2E8F0' }}>{c.name}</div>
                <div className="mono text-xs mt-0.5" style={{ color: '#64748B' }}>{mask(c.email)}</div>
                <div className="mono text-xs mt-1.5" style={{ color: '#334155' }}>{app?.emoji} {app?.name} - {c.plan}</div>
              </div>
            )
          })}
        </div>
      ...
      <div className="flex gap-1 flex-wrap" style={{ borderBottom: '1px solid #1E1E2E' }}>
        {tabs.map((tab) => {
          const isActive = activeTab === tab.id
          return (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className="mono text-xs px-3 py-2" style={{ color: isActive ? '#00D4AA' : '#64748B', borderBottom: isActive ? '2px solid #00D4AA' : '2px solid transparent', backgroundColor: 'transparent', marginBottom: '-1px' }}>
              {tab.label}
            </button>
          )
        })}
        <div className="ml-auto flex items-center gap-3 pb-1">
          <span className="mono text-xs" style={{ color: '#334155' }}>{filtered.length} customers - {activeCount} active - LTV ${totalLTV.toFixed(2)}</span>
        </div>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '600px' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #1E1E2E' }}>
                {['Name','Email','App','Plan','Joined','LTV','Status'].map((h) => <th key={h} className="px-4 py-3 text-left"><span className="label">{h}</span></th>)}
              </tr>
            </thead>
            <tbody>
              {filtered.map((c, i) => {
                const app = PORTFOLIO.find((a) => a.id === c.appId)
                const sc = SC[c.status] ?? SC.trial
                return (
                  <tr key={c.id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid #16162A' : undefined }}>
                    <td className="px-4 py-2.5"><span className="text-xs font-medium" style={{ color: '#E2E8F0' }}>{c.name}</span></td>
                    <td className="px-4 py-2.5"><span className="mono text-xs" style={{ color: '#64748B' }}>{mask(c.email)}</span></td>
                    <td className="px-4 py-2.5"><span className="mono text-xs" style={{ color: '#64748B' }}>{app?.emoji} {app?.name}</span></td>
                    <td className="px-4 py-2.5"><span className="mono text-xs" style={{ color: '#94A3B8' }}>{c.plan}</span></td>
                    <td className="px-4 py-2.5"><span className="mono text-xs" style={{ color: '#334155' }}>{new Date(c.joined).toLocaleDateString('en-US', { month:'short', day:'numeric', year:'2-digit' })}</span></td>
                    <td className="px-4 py-2.5"><span className="mono text-xs font-semibold" style={{ color: c.ltv > 0 ? '#10B981' : '#334155' }}>${c.ltv.toFixed(2)}</span></td>
                    <td className="px-4 py-2.5"><span className="mono text-xs px-2 py-0.5 rounded" style={{ backgroundColor: sc.bg, color: sc.text, fontSize: '0.6rem', textTransform: 'uppercase' }}>{c.status}</span></td>
                  </tr>
                )
              })}
            </tbody>
